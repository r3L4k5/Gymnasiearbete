...
func _on_area_mouse_exited():
	if not Input.is_action_pressed("Left-Click") and not mouse_is_hovering:
...
func pick_up_defence():
	...
	elif Input.is_action_pressed("Left-Click") and mouse_is_hovering:
		
		get_parent().global_position = get_viewport().get_mouse_position()
		
	elif Input.is_action_just_released("Left-Click") and mouse_is_hovering:
